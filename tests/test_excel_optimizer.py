...
class TestExcelOptimization:
    ...
    def test_export_dataframe_basic(self, sample_dataframe, temp_file):
        ...
        success = export_dataframe_to_excel_optimized(sample_dataframe, str(temp_file))
    ...
    def test_export_large_dataframe(self, temp_file):
        ...
        large_df = pd.DataFrame({"col_" + str(i): range(5000) for i in range(10)})

        success = export_dataframe_to_excel_optimized(large_df, str(temp_file))
        assert success is True

    def test_export_empty_dataframe(self, temp_file):
        """Test export of empty DataFrame."""
        empty_df = pd.DataFrame()
        success = export_dataframe_to_excel_optimized(empty_df, str(temp_file))
        assert success is True
    ...
    def test_export_multiple_sheets(self, sample_dataframe, temp_file):
        ...
        success = export_multiple_dataframes_to_excel(dataframes, str(temp_file))
        assert success is True
        assert temp_file.exists()
    ...
    def test_optimize_dataframe_numeric(self, sample_dataframe):
        ...
        df_opt = optimize_dataframe_for_excel(
            sample_dataframe, numeric_columns=["amount"]
        )
    ...
    def test_optimize_dataframe_datetime(self):
        ...
        df = pd.DataFrame(
            {"date": pd.date_range("2025-01-01", periods=10), "value": range(10)}
        )

        df_opt = optimize_dataframe_for_excel(df, datetime_columns=["date"])
    ...
    def test_optimize_dataframe_string(self):
        ...
        df = pd.DataFrame(
            {"category": ["A", "B", "A", "C", "B"] * 20, "value": range(100)}
        )

        df_opt = optimize_dataframe_for_excel(df, string_columns=["category"])
    ...
    def test_get_excel_export_info_large(self):
        ...
        large_df = pd.DataFrame({"col_" + str(i): range(10000) for i in range(20)})
