...
from config import get_config, SMTPConfig, EmailConfig
...
class TestEmailNotifierCredentials:
    ...
    def test_get_smtp_credentials_both_present(self):
        ...
        with patch.dict(
            os.environ,
            {"TDR_SMTP_USER": "test@gmail.com", "TDR_SMTP_PASS": "password123"},
        ):
    ...
    def test_send_notification_email_invalid_recipient(self):
        ...
        with patch(
            "utils.email_notifier.validate_email",
    ...
    def test_send_notification_email_invalid_server(self):
        ...
        with patch(
            "utils.email_notifier.validate_smtp_server",
            return_value=(False, "Invalid server"),
        ):
            result = send_notification_email(
    ...
    def test_send_notification_email_invalid_port(self):
        ...
        with patch(
            "utils.email_notifier.validate_smtp_port",
            return_value=(False, "Invalid port"),
        ):
            result = send_notification_email(
                smtp_server="smtp.gmail.com",
                smtp_port=99999,
...
class TestExcelHandlerBasic:
    ...
    def test_append_df_creates_new_sheet(self):
        ...
        df = pd.DataFrame(
            {"Filename": ["test.csv"], "Column1": [1], "Column2": ["data"]}
        )
        ...
        try:
            append_df_to_excel(filepath, df, "NewSheet")
            assert True
        finally:
            if filepath.exists():
                filepath.unlink()
...
class TestConfigIntegration:
    ...
    def test_email_config_class(self):
        ...
        email = EmailConfig(
            enabled=True, sender="test@gmail.com", recipients=["recipient@gmail.com"]
        )
...
class TestErrorHandling:
    ...
    def test_excel_handler_nonexistent_file_path(self):
        ...
        df = pd.DataFrame({"Filename": ["test.csv"], "Data": [123]})
        ...
        try:
            _ = append_df_to_excel(filepath, df, "Sheet")
            if filepath.exists():
                filepath.unlink()
        except Exception:
...
class TestDataIntegrity:
    ...
    def test_dataframe_types_preserved(self):
        ...
        try:
            append_df_to_excel(filepath, df, "TestSheet")
    ...
    def test_special_characters_in_data(self):
        ...
        try:
            append_df_to_excel(filepath, df, "Sheet")
