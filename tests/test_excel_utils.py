...
@pytest.mark.parametrize(
    "input_val, ref_date, is_date_only, expected",
    [
...
def test_parse_excel_datetime_various_inputs(
    input_val, ref_date, is_date_only, expected
):
    assert (
        parse_excel_datetime(
            input_val, date_val_for_time=ref_date, is_just_date=is_date_only
        )
        == expected
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            False,
            date(2023, 7, 7),
        ),  # Nếu input là date, is_date_only=False vẫn trả date
        (date(2023, 7, 7), None, True, date(2023, 7, 7)),
        ("15:45", date(2023, 1, 1), False, datetime(2023, 1, 1, 15, 45)),
        ("08:00", date(2023, 10, 20), False, datetime(2023, 10, 20, 8, 0)),
        (
        ...
        ),  # 0.75 của một ngày là 6 PM
        (45321.5, None, False, datetime(2024, 1, 30, 12, 0, 0)),  # Excel serial date
        (45321, None, True, date(2024, 1, 30)),
        ("Invalid String", None, False, None),
        (None, None, False, None),
        ("10/07", date(2023, 1, 1), True, date(2023, 7, 10)),  # dd/mm với ref year
        (
            "Jul 10",
            date(2023, 1, 1),
            True,
            date(2023, 7, 10),
        ),  # Giả sử hàm hỗ trợ "Mon Day"
    ],
)
def test_parse_excel_datetime_basic_inputs(input_val, ref_date, is_date_only, expected):
    assert (
        parse_excel_datetime(
            input_val, date_val_for_time=ref_date, is_just_date=is_date_only
        )
...
def test_classify_error_code_logic(monkeypatch):
    ...
    assert (
        classify_error_code("kẹt xe ngoài cảng")[1]
        == MockExcelUtilsConfig.DELAY_ERROR_UNKNOWN_TYPE
    )  # Không có trong keywords
    assert (
        classify_error_code("p - Lý do khác")[1]
        == MockExcelUtilsConfig.DELAY_ERROR_UNKNOWN_TYPE
    )  # 'p' không trong config
...
@pytest.mark.parametrize(
    "input_val, expected_hours",
    [
        (timedelta(hours=1, minutes=45), 1.75),
        (time(2, 15, 0), 2.25),
        (
            datetime(2023, 1, 1, 3, 30, 0),
            3.50,
        ),  # Nếu truyền datetime, nó sẽ lấy phần time
        ("01:10", 1.17),  # 1 + 10/60
        ("0:30:00", 0.5),
