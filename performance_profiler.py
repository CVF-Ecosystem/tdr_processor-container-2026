...
class PerformanceProfiler:
    ...
    def record_result(self, operation: str, execution_time: float) -> None:
        ...
        """
        result = PerformanceResult(operation=operation, execution_time=execution_time)
    ...
    def save_results_json(self, filename: str = "performance_results.json") -> str:
        ...
        data = {
            ...
            "summary": {
                ...
                "average_time": sum(r.execution_time for r in self.results)
                / len(self.results)
                if self.results
                else 0,
            },
    ...
    def print_summary(self) -> None:
        ...
        print(
            f"Min Time: {min(self.results, key=lambda r: r.execution_time).execution_time:.4f}s"
        )
        print(
            f"Max Time: {max(self.results, key=lambda r: r.execution_time).execution_time:.4f}s"
        )

        print("\nTop 10 Slowest Operations:")
        sorted_results = sorted(
            self.results, key=lambda r: r.execution_time, reverse=True
        )
        for i, result in enumerate(sorted_results[:10], 1):
            percent = (result.execution_time / total_time) * 100
            print(
                f"  {i:2}. {result.operation:40} {result.execution_time:.4f}s ({percent:6.2f}%)"
            )
...
def profile_data_extraction(profiler: PerformanceProfiler) -> None:
    ...
    start = time.time()
    _ = extractor.extract_vessel_info()
    elapsed = time.time() - start
...
def profile_dataframe_operations(profiler: PerformanceProfiler) -> None:
    ...
    profiler.record_result(
        f"format_dataframe_numeric_columns (n={size}, cols=2)", elapsed
    )
    ...
    elapsed = time.time() - start
    profiler.record_result(
        f"DataFrame.groupby().agg() (n={size}, groups={len(result)})", elapsed
    )
...
def profile_excel_writing(profiler: PerformanceProfiler) -> None:
    ...
    # Create test dataframe
    size = 5000
    test_df = pd.DataFrame({f"col_{i}": range(size) for i in range(10)})
