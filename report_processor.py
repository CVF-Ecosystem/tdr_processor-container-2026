...
from datetime import (
    datetime,
)  # For type hints (removed 'time' to avoid naming conflict)
...
class ReportProcessor:
        ...
        if vessel_master_filepath.exists():
            try:
                wb = openpyxl.load_workbook(
                    vessel_master_filepath, read_only=True, data_only=True
                )
                ...
                logging.warning(
                    f"Không thể đọc {config.VESSEL_MASTER_FILE} để kiểm tra file đã xử lý: {e}"
                )
    ...
    def _process_single_file(self, filepath: Path) -> bool:
        ...
        try:
            ...
            container_list_long: List[Dict[str, Any]] = (
                extractor.extract_container_details()
            )
            ...
            if not df_qc.empty:
                ...
                qc_productivity_df["Net working (hrs)"] = qc_productivity_df.apply(
                    lambda row: round(
                        max(
                            0,
                            row.get("Gross working (hrs)", 0.0)
                            - row.get("Delay times (hrs)", 0.0),
                        ),
                        2,
                    ),
                    axis=1,
                ...
                qc_productivity_df["Net moves/h"] = qc_productivity_df.apply(
                    lambda row: round(row["Total Conts"] / row["Net working (hrs)"], 1)
                    if row["Net working (hrs)"] > 0
                    else 0.0,
                    axis=1,
                )
            ...
            if not df_qc.empty:
                ...
                if not delay_details_df.empty:
                    ...
                    delay_details_df["QC_No_Normalized"] = delay_details_df[
                        "QC No."
                    ].apply(_normalize_qc_name)
                    qc_actual_delays = delay_details_df.groupby("QC_No_Normalized")[
                        "Duration (hrs)"
                    ].sum()
                ...
                def calculate_operator_qc_metrics(row):
                    ...
                    row["Net working (hrs)"] = round(
                        max(0, row.get("Gross working (hrs)", 0.0) - actual_delay), 2
                    )

                    if row["Net working (hrs)"] > 0 and row.get("Total Conts", 0) > 0:
                        row["Net moves/h"] = round(
                            row["Total Conts"] / row["Net working (hrs)"], 1
                        )
                    else:
                        row["Net moves/h"] = 0.0
                    return row

                df_qc_operator_final = qc_operator_df.apply(
                    calculate_operator_qc_metrics, axis=1
                )
    ...
    def _process_container_data(self, df_container_long_raw: pd.DataFrame):
        ...
        condition_total_ops = temp_df_c["OperationType"].isin(
            [config.OP_TOTAL_DIS, config.OP_TOTAL_LOAD]
        )
        temp_df_c = temp_df_c[~condition_total_ops]
        condition_dis_load_all_ports = temp_df_c["OperationType"].isin(
            [config.OP_DISCHARGE, config.OP_LOADING]
        ) & (temp_df_c["Port"] == config.PORT_ALL)
        temp_df_c = temp_df_c[~condition_dis_load_all_ports]
        condition_shifting_all_ports = temp_df_c["OperationType"].isin(
            [config.OP_SHIFTING_DIS, config.OP_SHIFTING_LOAD]
        ) & (temp_df_c["Port"] == config.PORT_ALL)
        ...
        if not df_container_long_filtered.empty:
            try:
                df_container_long_filtered["ContainerTypeSize"] = (
                    df_container_long_filtered["ContainerCategory"]
                    + "_"
                    + df_container_long_filtered["ContainerSize"].astype(str)
                )
                df_container_pivot = pd.pivot_table(
                    df_container_long_filtered,
                    values="Quantity",
                ...
                quantity_cols_in_wide = [
                    col
                    for col in df_container_wide.columns
                    if col not in config.CONTAINER_WIDE_BASE_COLS
                ]
                if quantity_cols_in_wide:
                    df_container_wide[config.CONTAINER_WIDE_TOTAL_COLS_SUFFIX[0]] = (
                        df_container_wide[quantity_cols_in_wide].sum(axis=1)
                    )
                    df_container_wide[config.CONTAINER_WIDE_TOTAL_COLS_SUFFIX[1]] = 0
                    for col_name in quantity_cols_in_wide:
                        if "_20" in col_name:
                            df_container_wide[
                                config.CONTAINER_WIDE_TOTAL_COLS_SUFFIX[1]
                            ] += df_container_wide[col_name] * 1
                        elif "_40" in col_name:
                            df_container_wide[
                                config.CONTAINER_WIDE_TOTAL_COLS_SUFFIX[1]
                            ] += df_container_wide[col_name] * 2
                ...
                remaining_value_cols_in_wide = [
                    col
                    for col in quantity_cols_in_wide
                    if col not in ordered_value_cols
                ]
                final_container_wide_col_order = (
                    config.CONTAINER_WIDE_BASE_COLS
                    + ordered_value_cols
                    + remaining_value_cols_in_wide
                    + config.CONTAINER_WIDE_TOTAL_COLS_SUFFIX
                )
                final_container_wide_col_order = [
                    col
                    for col in final_container_wide_col_order
                    if col in df_container_wide.columns
                ]
                df_container_wide = df_container_wide[final_container_wide_col_order]

                df_container_wide["OpSortOrder"] = (
                    df_container_wide["OperationType"]
                    .map(config.OPERATION_TYPE_SORT_ORDER)
                    .fillna(999)
                )
                df_container_wide = df_container_wide.sort_values(
                    by=["Filename", "Vessel Name", "Voyage", "OpSortOrder", "Port"],
                    ascending=[True, True, True, True, True],
                )
                df_container_wide = df_container_wide.drop(columns=["OpSortOrder"])
            except Exception as e_pivot:
                logging.error(
                    f"Lỗi pivot/sắp xếp container details: {e_pivot}", exc_info=True
                )
    ...
    def _save_sqlite(self, final_dataframes: dict) -> None:
        ...
        try:
            ...
            for table_name, excel_key in table_map.items():
                if (
                    excel_key in final_dataframes
                    and final_dataframes[excel_key] is not None
                ):
    ...
    def _save_csv_files(self, final_dataframes: dict):
        ...
        for csv_name, excel_name in csv_map.items():
            if (
                excel_name in final_dataframes
                and final_dataframes[excel_name] is not None
            ):
                ...
                logging.info(
                    f"Đã xuất file CSV: {csv_name} với {len(df_to_write_csv)} dòng."
                )
            else:
                logging.info(
                    f"Bỏ qua xuất file CSV '{csv_name}' vì không có dữ liệu tương ứng."
                )

    def _aggregate_and_save_data(self) -> str:
        if (
            not self.all_vessel_dfs
            and not self.all_qc_dfs
        ...
        df_vessel_new = (
            pd.concat(self.all_vessel_dfs, ignore_index=True)
            if self.all_vessel_dfs
            else pd.DataFrame()
        )
        df_qc_new = (
            pd.concat(self.all_qc_dfs, ignore_index=True)
            if self.all_qc_dfs
            else pd.DataFrame()
        )
        df_qc_operator_new = (
            pd.concat(self.all_qc_operator_dfs, ignore_index=True)
            if self.all_qc_operator_dfs
            else pd.DataFrame()
        )
        df_delay_new = (
            pd.concat(self.all_delay_dfs, ignore_index=True)
            if self.all_delay_dfs
            else pd.DataFrame()
        )
        df_container_long_new = (
            pd.concat(self.all_container_long_dfs, ignore_index=True)
            if self.all_container_long_dfs
            else pd.DataFrame()
        )
        ...
        def safe_select_columns(df, col_renames, col_order):
            ...
            if not existing_cols:
                logging.warning(
                    f"No columns from order list found in DataFrame. Columns available: {df.columns.tolist()}"
                )
                return df

            return df[existing_cols]

        final_df_vessel = safe_select_columns(
            df_vessel_new,
            config.VESSEL_COL_RENAMES_OUTPUT,
            config.VESSEL_COLS_OUTPUT_ORDER,
        )
        final_df_qc = safe_select_columns(
            df_qc_new, config.QC_COL_RENAMES_OUTPUT, config.QC_COLS_OUTPUT_ORDER
        )
        ...
        df_qc_operator_new = df_qc_operator_new.drop(
            columns=["Delay times (hrs)"], errors="ignore"
        )
        final_df_qc_operator = safe_select_columns(
            df_qc_operator_new,
            config.QC_OPERATOR_COL_RENAMES_OUTPUT,
            QC_OPERATOR_COLS_OUTPUT_ORDER_NEW,
        )
        # <<< KẾT THÚC THAY ĐỔI >>>

        final_df_delay = safe_select_columns(
            df_delay_new, {}, config.DELAY_DETAILS_COLS_OUTPUT_ORDER
        )
        final_df_container_long, final_df_container_wide = self._process_container_data(
            df_container_long_new
        )

        # --- Bước 3: Ghi file Excel ---
        output_map_excel = {
            config.VESSEL_MASTER_FILE: (
                final_df_vessel,
                config.VESSEL_SUMMARY_SHEET,
                {
                    "datetime_cols_to_format": config.VESSEL_DATETIME_COLS_FORMAT,
                    "index": False,
                },
            ),
            config.QC_MASTER_FILE: (
                final_df_qc,
                config.QC_PRODUCTIVITY_SHEET,
                {"time_cols_to_format": config.QC_TIME_COLS_FORMAT, "index": False},
            ),
            config.QC_OPERATOR_MASTER_FILE: (
                final_df_qc_operator,
                config.QC_OPERATOR_PRODUCTIVITY_SHEET,
                {"index": False},
            ),
            config.DELAY_MASTER_FILE: (
                final_df_delay,
                config.DELAY_DETAILS_SHEET,
                {
                    "datetime_cols_to_format": config.DELAY_DATETIME_COLS_FORMAT,
                    "index": False,
                },
            ),
            config.CONTAINER_MASTER_WIDE_FILE: (
                final_df_container_wide,
                config.CONTAINER_SUMMARY_WIDE_SHEET,
                {"index": False},
            ),
            config.CONTAINER_MASTER_LONG_FILE: (
                final_df_container_long,
                config.CONTAINER_SUMMARY_LONG_SHEET,
                {"index": False},
            ),
        }
    ...
    def process_tdr_files(
        ...
        if not self._processing_lock.acquire(blocking=False):
            logging.warning(
                "ReportProcessor: Processing already in progress. Skipping concurrent request."
            )
            return {
    ...
    def _process_tdr_files_internal(
        self,
        input_file_paths: list[Path],
        update_status_callback=None,
        update_progress_callback=None,
        ...
        if not overwrite:
            ...
            logging.info(
                f"Chế độ 'Chỉ xử lý file mới'. Tìm thấy {len(existing_filenames)} file đã có trong master data."
            )
        else:
            logging.info(
                "Chế độ 'Xử lý lại' được bật. Sẽ xử lý tất cả các file được chọn."
            )
        ...
        for idx, filepath_obj in enumerate(input_file_paths):
            ...
            if update_status_callback:
                update_status_callback(
                    f"✅Processing...: {filepath_obj.name} ({idx+1}/{total_files})"
                )
        ...
        try:
            ...
            logging.info(
                f"Tổng thời gian xử lý {self.processed_files_count} file mới: {time_taken:.2f} giây"
            )
            ...
            time_taken = end_total_time - start_total_time
            logging.error(
                f"Lỗi nghiêm trọng khi tổng hợp hoặc lưu dữ liệu: {e_agg}",
                exc_info=True,
            )
