...
def detect_and_move_duplicates(
    input_dir: Path, duplicates_folder_name: str = "duplicates"
        ...
        if len(files) == 1:
            ...
            for dup_file in sorted_files[1:]:
                ...
                try:
                    ...
                    logging.warning(
                        f"Moved duplicate file: {dup_file.name} -> {duplicates_folder_name}/"
                    )
    ...
    if duplicate_files:
        logging.info(
            f"Detected and moved {len(duplicate_files)} duplicate file(s) to '{duplicates_folder_name}' folder"
        )
...
def process_tdr_files(
    ...
    setup_project_directories(
        Path.cwd(), ["outputs", "outputs/data_csv", "outputs/data_excel"]
    )
...
def auto_process_input_folder(
    ...
    duplicate_files = []
    if check_duplicates:
        valid_files, duplicate_files = detect_and_move_duplicates(
            input_dir, duplicates_folder_name
        )
        if duplicate_files:
            logging.warning(
                f"[AutoProcess] Moved {len(duplicate_files)} duplicate file(s) to '{duplicates_folder_name}' folder"
            )
    else:
    ...
    result = process_tdr_files(
        files=validated_files, output_dir=output_dir, overwrite=overwrite
    )
...
def get_processing_summary(output_dir: Optional[Path] = None) -> Dict[str, Any]:
    ...
    summary = {"csv_files": [], "excel_files": [], "last_modified": None}
...
def process_tdr_files_parallel(
    ...
    if len(files) <= 3 or max_workers <= 1:
        logging.info(
            f"[ParallelProcess] Small batch ({len(files)} files), using sequential processing"
        )
    ...
    logging.info(
        f"[ParallelProcess] Starting parallel processing of {len(files)} files with {max_workers} workers"
    )
    ...
    def process_chunk(chunk: List[Path]) -> Dict[str, Any]:
        ...
        processor = ReportProcessor(output_dir=output_dir)
        return processor.process_tdr_files(chunk, overwrite=overwrite)
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_chunk):
            ...
            try:
                ...
                logging.error(
                    f"[ParallelProcess] Chunk processing failed: {e}", exc_info=True
                )
