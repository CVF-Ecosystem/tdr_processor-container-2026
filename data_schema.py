...
VESSEL_SUMMARY_SCHEMA = TableSchema(
    ...
    columns=[
        ColumnSchema(
            ...
            DataType.STRING,
            nullable=False,
            description="Source TDR filename",
        ),
        ColumnSchema(
            "Vessel Name", DataType.STRING, nullable=False, description="Vessel name"
        ),
        ColumnSchema(
            "Voyage", DataType.STRING, nullable=False, description="Voyage number"
        ),
        ColumnSchema(
            "Operator",
            DataType.STRING,
            nullable=True,
            description="Shipping line operator",
        ),
        ColumnSchema(
            "Berth", DataType.STRING, nullable=True, description="Berth location"
        ),
        ColumnSchema(
            "ATB",
            DataType.DATETIME,
        ...
        ColumnSchema(
            ...
            DataType.FLOAT,
            nullable=True,
            description="Total time at berth (ATD - ATB)",
        ),
        ColumnSchema(
            "Gross Working (hrs)",
            DataType.FLOAT,
            nullable=True,
            description="Total working time including delays",
        ),
        ColumnSchema(
            "Net Working (hrs)",
            DataType.FLOAT,
            nullable=True,
            description="Working time excluding non-port delays",
        ),
        ColumnSchema(
            "Grand Total Conts",
            DataType.INTEGER,
            nullable=True,
            description="Total container moves",
        ),
        ColumnSchema(
            "Grand Total TEUs",
            DataType.FLOAT,
            nullable=True,
            description="Total TEU count",
        ),
        ColumnSchema(
            "Vessel Moves/Portstay Hour",
            DataType.FLOAT,
            nullable=True,
            description="Productivity: Conts/Portstay",
        ),
        ColumnSchema(
            "Vessel Moves/Gross Hour",
            DataType.FLOAT,
            nullable=True,
            description="Productivity: Conts/Gross Working",
        ),
        ColumnSchema(
            "Vessel Moves/Net Hour",
            DataType.FLOAT,
            nullable=True,
            description="Productivity: Conts/Net Working",
        ),
    ],
)
...
QC_PRODUCTIVITY_SCHEMA = TableSchema(
    ...
    columns=[
        ...
        ColumnSchema(
            ...
            DataType.STRING,
            nullable=False,
            description="Normalized QC identifier (GC01, GW01, etc)",
        ),
        ...
        ColumnSchema(
            ...
            DataType.FLOAT,
            nullable=True,
            description="Total stop/delay time",
        ),
        ...
        ColumnSchema(
            ...
            DataType.FLOAT,
            nullable=True,
            description="Total Conts / Gross working",
        ),
        ColumnSchema(
            "Net moves/h",
            DataType.FLOAT,
            nullable=True,
            description="Total Conts / Net working",
        ),
    ],
)
...
QC_OPERATOR_PRODUCTIVITY_SCHEMA = TableSchema(
    ...
    columns=[
        ...
        ColumnSchema(
            ...
            DataType.FLOAT,
            nullable=True,
            description="Non-operator delay time",
        ),
        ColumnSchema(
            "Net working (hrs)",
            DataType.FLOAT,
            nullable=True,
            description="Gross - operator-responsible delays only",
        ),
        ColumnSchema("Discharge Conts", DataType.INTEGER, nullable=True),
        ColumnSchema("Load Conts", DataType.INTEGER, nullable=True),
        ColumnSchema("Shifting Conts", DataType.INTEGER, nullable=True),
...
DELAY_EVENTS_SCHEMA = TableSchema(
    ...
    columns=[
        ...
        ColumnSchema("Voyage", DataType.STRING, nullable=False),
        ColumnSchema(
            "QC No.", DataType.STRING, nullable=False, description="Normalized QC name"
        ),
        ColumnSchema(
            "Stop Category",
            DataType.STRING,
            nullable=True,
            description="Stop-1, Stop-2, Stop-3",
        ),
        ColumnSchema(
            "From Time",
            DataType.DATETIME,
        ...
        ColumnSchema(
            "To Time", DataType.DATETIME, nullable=True, description="Delay end time"
        ),
        ColumnSchema(
            "Duration (hrs)",
            DataType.FLOAT,
        ...
        ColumnSchema(
            "Error Code", DataType.STRING, nullable=True, description="d, i, or m"
        ),
        ColumnSchema(
            "Error Type",
            DataType.STRING,
            nullable=True,
            description="Terminal/Non-Terminal Convenience",
        ),
        ColumnSchema(
            "Remark", DataType.STRING, nullable=True, description="Original remark text"
        ),
    ],
)
...
CONTAINER_DETAILS_SCHEMA = TableSchema(
    ...
    columns=[
        ...
        ColumnSchema(
            ...
            DataType.STRING,
            nullable=False,
            description="Discharge/Load/Shifting",
        ),
        ColumnSchema("Size", DataType.STRING, nullable=False, description="20/40/45"),
        ColumnSchema(
            "Category", DataType.STRING, nullable=False, description="Full/Empty"
        ),
        ColumnSchema(
            "Conts", DataType.INTEGER, nullable=True, description="Container count"
        ),
        ColumnSchema(
            "TEUs", DataType.FLOAT, nullable=True, description="TEU equivalent"
        ),
    ],
)
        ...
        if not col.nullable and col.name in df.columns:
            ...
            if null_count > 0:
                result["null_violations"].append(
                    f"{col.name}: {null_count} null values"
                )

    # Determine validity
    result["valid"] = (
        len(result["missing_columns"]) == 0 and len(result["null_violations"]) == 0
    )
...
if PYDANTIC_AVAILABLE:
    ...
    class VesselSummaryModel(BaseModel):
        ...
        gross_working_hrs: Optional[float] = Field(
            None, alias="Gross Working (hrs)", ge=0
        )
        ...
        @field_validator(
            "portstay_hrs", "gross_working_hrs", "net_working_hrs", mode="before"
        )
        @classmethod
    ...
    def validate_vessel_records(records: list) -> tuple[list, list]:
        ...
        for i, record in enumerate(records):
            try:
                ...
                errors.append(
                    {"index": i, "record": record.get("Filename", "?"), "error": str(e)}
                )
        return valid, errors
