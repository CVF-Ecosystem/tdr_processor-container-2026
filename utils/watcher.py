...
class RobustFileHandler(FileSystemEventHandler):
    ...
    def _queue_file(self, filepath: Path) -> None:
        ...
        if (
            filepath in self.last_event_time
            and current_time - self.last_event_time[filepath] < 2.0
        ):
            return
...
class Watcher:
    ...
    def __init__(self, path_to_watch, queue: Optional[Queue] = None) -> None:
        ...
        """
        self.path: Path = (
            path_to_watch if isinstance(path_to_watch, Path) else Path(path_to_watch)
        )
