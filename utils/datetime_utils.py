...
try:
    from utils.excel_utils import parse_excel_datetime, excel_to_time
...
def extract_datetime_from_cell(
    ...
    try:
        ...
        if raw_value is None or (
            ...
            logging.debug(
                f"extract_datetime_from_cell ({context}): Empty cell at row {row}, col {column}"
            )
            return None

        parsed_datetime = parse_excel_datetime(
            raw_value, reference_date, False, context
        )
        return parsed_datetime

    except Exception as e:
        logging.warning(
            f"extract_datetime_from_cell ({context}): Error parsing cell ({row}, {column}): {e}"
        )
        return None


def extract_date_from_cell(
    worksheet: Worksheet, row: int, column: int, context: str = ""
) -> Optional[date]:
    """
    ...
    """
    try:
        datetime_result = extract_datetime_from_cell(
            worksheet, row, column, None, context
        )
        ...
        logging.warning(
            f"extract_date_from_cell ({context}): Error at row {row}, col {column}: {e}"
        )
        return None
...
def extract_time_from_cell(
    ...
    try:
        ...
        logging.warning(
            f"extract_time_from_cell ({context}): Error parsing cell ({row}, {column}): {e}"
        )
        return None
...
def parse_datetime_range(
    ...
    """
    try:
        start_datetime = parse_excel_datetime(
            start_value, reference_date, False, f"{context} - Start"
        )
        end_datetime = parse_excel_datetime(
            end_value, reference_date, False, f"{context} - End"
        )
        ...
        if (
            ...
            logging.debug(
                f"parse_datetime_range ({context}): End time was earlier than start, added 1 day"
            )
        ...
        logging.warning(
            f"parse_datetime_range ({context}): Error parsing datetime range: {e}"
        )
        return None, None
...
def calculate_duration_hours(
    ...
    try:
        if not (
            isinstance(start_datetime, datetime) and isinstance(end_datetime, datetime)
        ):
            return 0.0

        if end_datetime < start_datetime:
            logging.warning(
                f"calculate_duration_hours ({context}): End time before start time"
            )
            return 0.0
        ...
        logging.warning(
            f"calculate_duration_hours ({context}): Error calculating duration: {e}"
        )
        return 0.0


def shift_datetime_to_reference_date(
    time_value: Optional[time], reference_date: Optional[date], context: str = ""
) -> Optional[datetime]:
    """
    ...
    try:
        if not isinstance(time_value, time) or not isinstance(reference_date, date):
            logging.debug(
                f"shift_datetime_to_reference_date ({context}): Invalid input types"
            )
            return None

        combined_datetime = datetime.combine(reference_date, time_value)
        logging.debug(
            f"shift_datetime_to_reference_date ({context}): Combined {time_value} with {reference_date}"
        )
        return combined_datetime

    except Exception as e:
        logging.warning(
            f"shift_datetime_to_reference_date ({context}): Error combining datetime: {e}"
        )
        return None
