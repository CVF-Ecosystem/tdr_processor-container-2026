...
def apply_qc_calculations(
    ...
    result_df["Net working (hrs)"] = result_df.apply(
        lambda row: round(
            max(0, row.get("Gross working (hrs)", 0.0) - row.get(delay_column, 0.0)), 2
        ),
        axis=1,
    )
    ...
    result_df["Net moves/h (QC)"] = result_df.apply(
        lambda row: round(row[containers_column] / row["Net working (hrs)"], 1)
        if row["Net working (hrs)"] > 0 and row.get(containers_column, 0) > 0
        else 0.0,
        axis=1,
    )
...
def calculate_vessel_kpis(vessel_info: Dict[str, Any]) -> Dict[str, float]:
    ...
    if net_working_hours > 0 and grand_total_containers > 0:
        kpis["Vessel Moves/Net Hour"] = round(
            grand_total_containers / net_working_hours, 1
        )
    else:
    ...
    if gross_working_hours > 0 and grand_total_containers > 0:
        kpis["Vessel Moves/Gross Hour"] = round(
            grand_total_containers / gross_working_hours, 1
        )
    else:
    ...
    if portstay_hours > 0 and grand_total_containers > 0:
        kpis["Vessel Moves/Portstay Hour"] = round(
            grand_total_containers / portstay_hours, 1
        )
    else:
...
def format_dataframe_numeric_columns(
    ...
    logging.debug(
        f"Formatted {len(numeric_columns)} numeric columns to {decimal_places} decimal places"
    )
    return result_df
...
def aggregate_multiple_dataframes(
    ...
    if sort_by and sort_by in combined_df.columns:
        ...
        logging.info(
            f"Aggregated {len(dataframes)} DataFrames and sorted by '{sort_by}'"
        )
    else:
