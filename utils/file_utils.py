...
def is_file_locked(filepath) -> bool:
    ...
    try:
        ...
        logging.warning(
            f"KIỂM TRA FILE: File '{filepath.name}' đang bị khóa hoặc không có quyền truy cập."
        )
        return True
    except Exception as e:
        logging.error(
            f"KIỂM TRA FILE: Lỗi không xác định khi kiểm tra file '{filepath.name}': {e}"
        )
...
def backup_file(file_path: Path) -> None:
    ...
    try:
        ...
        logging.error(
            f"BACKUP: Lỗi khi sao lưu file '{file_path.name}': {e}", exc_info=True
        )
