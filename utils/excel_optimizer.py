...
def export_dataframe_to_excel_optimized(
    ...
    try:
            ...
            for col_idx, value in enumerate(row):
                if isinstance(value, (int, float)):
                    if format_numbers and isinstance(value, float):
                        worksheet.write(
                            row_idx, col_idx + col_offset, value, number_format
                        )
                    else:
                        worksheet.write(
                            row_idx, col_idx + col_offset, value, text_format
                        )
                else:
        ...
        if auto_width:
            for col_idx, col_name in enumerate(dataframe.columns):
                max_length = max(
                    len(str(col_name)), dataframe[col_name].astype(str).str.len().max()
                )
                worksheet.set_column(
                    col_idx + col_offset, col_idx + col_offset, min(max_length + 2, 50)
                )
...
def export_multiple_dataframes_to_excel(
    ...
    try:
        ...
        # Define formats
        header_format = workbook.add_format(
            {"bold": True, "border": 1, "bg_color": "#D3D3D3"}
        )
        number_format = workbook.add_format({"num_format": "0.00"})
        text_format = workbook.add_format({"border": 1})
        ...
        for sheet_idx, (sheet_name, df) in enumerate(dataframes.items()):
            ...
            for col_idx, col_name in enumerate(df.columns):
                max_length = max(
                    len(str(col_name)), df[col_name].astype(str).str.len().max()
                )
        ...
        logging.info(
            f"[Excel] Exported {total_rows} rows across {len(dataframes)} sheets to {filepath}"
        )
        return True
...
def get_excel_export_info(dataframe: pd.DataFrame) -> Dict[str, any]:
    ...
    return {
        ...
        "estimated_xlsx_kb": (
            len(dataframe) * len(dataframe.columns) * 0.1
        ),  # Rough estimate
    }
