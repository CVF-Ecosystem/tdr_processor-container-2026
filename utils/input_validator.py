    ...
    if base_directory:
        try:
            ...
            if not str(resolved_path).startswith(str(base_dir)):
                return (
                    False,
                    "Path traversal detected (resolved path outside base directory)",
                )
...
def validate_excel_file(file_path: str, max_size_mb: int = 100) -> Tuple[bool, str]:
    ...
    if path_obj.suffix.lower() not in allowed_extensions:
        return (
            False,
            f"File extension not allowed. Expected: {', '.join(allowed_extensions)}",
        )
    ...
    if file_size_mb > max_size_mb:
        return (
            False,
            f"File size exceeds maximum ({max_size_mb} MB). Actual: {file_size_mb:.1f} MB",
        )
        ...
        try:
            with open(path_obj, "rb") as f:
                ...
                if header[:2] != b"PK":
                    return (
                        False,
                        "File is not a valid Excel file (invalid ZIP signature)",
                    )
...
def validate_smtp_port(port: int) -> Tuple[bool, str]:
    ...
    Accepts standard SMTP ports: 25, 587 (STARTTLS), 465 (SMTPS), 2525 (alternate).
    ...
    if port not in allowed_ports:
        return False, "Invalid SMTP port. Allowed ports are 25, 465, 587, 2525"
