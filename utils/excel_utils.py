...
def col_letter_to_index(letter: str) -> int | None:
    ...
    for char_val in letter:
        if "A" <= char_val <= "Z":  # Kiểm tra ký tự hợp lệ
            ...
            logging.debug(
                f"col_letter_to_index: Invalid character '{char_val}' in input '{letter}'."
            )
...
def classify_error_code(error_code_and_remark: str | None) -> tuple[str | None, str]:
    ...
    if error_code:
        if error_code in config.DELAY_ERROR_CODE_CLASSIFICATION.get(
            "Terminal Convenience", []
        ):
            return error_code, "Terminal Convenience"
        elif error_code in config.DELAY_ERROR_CODE_CLASSIFICATION.get(
...
def find_label_row_col(
    ws,
    label_text: str,
    search_cols: tuple[int, int] = (1, 20),
    start_row: int = 1,
    max_search_rows: int | None = None,
    partial_match: bool | None = None,
    specific_col_letter: str | None = None,
    ...
    if specific_col_letter:
        ...
        if col_idx:
            ...
            logging.warning(
                f"find_label: Invalid col_letter '{specific_col_letter}' for '{label_text}'. Defaulting search range."
            )

    _max_search_rows = (
        max_search_rows
        if max_search_rows is not None
        else config.MAX_SEARCH_ROWS_DEFAULT
    )
    _partial_match = (
        partial_match if partial_match is not None else config.PARTIAL_MATCH_DEFAULT
    )
    ...
    for r in range(start_row, end_row_to_search):
        for c in range(
            col_to_search_start, min(ws.max_column + 1, col_to_search_end + 1)
        ):
            ...
            if cell.value:
                ...
                if _partial_match:
                    ...
                    if normalized_label_text == normalized_cell_val:
                        return r, c
    logging.info(
        f"find_label: Not found '{label_text}' (Cols {get_column_letter(col_to_search_start)}-{get_column_letter(col_to_search_end)}, Rows {start_row}-{end_row_to_search-1}, partial={_partial_match})."
    )
    return None, None
...
def excel_to_time(excel_time_val) -> time | None:
    ...
    if isinstance(excel_time_val, time):
        return excel_time_val
    if isinstance(excel_time_val, datetime):
        return excel_time_val.time()
    if isinstance(excel_time_val, (int, float)):
        if 0 <= excel_time_val < 1:  # Excel time as fraction of a day
            ...
            try:
                ...
                logging.warning(
                    f"excel_to_time: Cannot convert numeric value '{excel_time_val}' to time."
                )
                return None
    logging.debug(
        f"excel_to_time: Unsupported type '{type(excel_time_val)}' for value '{excel_time_val}'"
    )
    return None
...
def _parse_time_string_with_strptime(time_str: str, context: str = "") -> time | None:
    ...
    time_formats = [
        ...
        "%H:%M",
        "%I:%M:%S %p",
        "%I:%M %p",  # Hỗ trợ 12-hour format với AM/PM
    ]
    for fmt_t in time_formats:
        try:
            return datetime.strptime(
                time_str.upper(), fmt_t
            ).time()  # .upper() để AM/PM thành AM/PM
        except ValueError:
            continue
    logging.warning(
        f"{log_prefix}: Could not parse time_str: '{time_str}' using formats {time_formats}"
    )
    return None


def _parse_date_string_with_strptime(
    date_str: str, year_to_use: int, context: str = ""
) -> date | None:
    """
    ...
    date_formats_with_year = [
        ...
        "%Y-%m-%d",
        "%d-%b-%Y",
        "%d-%B-%Y",  # Ví dụ: 10-Jul-2023, 10-July-2023
        "%b %d, %Y",
        "%B %d, %Y",  # Ví dụ: Jul 10, 2023, July 10, 2023
    ]
    ...
    date_formats_no_year = ["%d/%m", "%m/%d", "%d-%b", "%d-%B", "%b %d", "%B %d"]
    for fmt_d_no_year in date_formats_no_year:
        try:
            ...
            return (
                datetime.strptime(date_str, fmt_d_no_year)
                .replace(year=year_to_use)
                .date()
            )
        except ValueError:
            continue
    logging.warning(
        f"{log_prefix}: Could not parse date_str: '{date_str}' using formats."
    )
    return None


def _parse_datetime_from_string_logic(
    dt_s: str, date_ref: date | None, is_date_only: bool, ctx: str = ""
) -> datetime | date | None:
    """
    ...
    dt_s_processed = re.sub(
        r"thg\s*\.?\s*", "-", dt_s_processed, flags=re.IGNORECASE
    )  # "thg 7" -> " -7" (cần xử lý thêm)
        ...
        for fmt in datetime_formats_full:
            try:
                ...
                parsed_dt = datetime.strptime(
                    dt_s_processed.upper().replace(" SA", " AM").replace(" CH", " PM"),
                    fmt,
                )
                logging.info(
                    f"{log_p} (SUCCESS - full format): Parsed '{dt_s_processed}' to {parsed_dt}"
                )
                return parsed_dt
            except ValueError:
                pass
    ...
    # Nếu chỉ là ngày
    if is_date_only:
        year_to_use_for_date = (
            date_ref.year
            if isinstance(date_ref, (date, datetime))
            else datetime.now().year
        )
        parsed_date_only = _parse_date_string_with_strptime(
            dt_s_processed, year_to_use_for_date, ctx + "_date_only_mode"
        )
        if parsed_date_only:
            logging.info(
                f"{log_p} (SUCCESS - date only mode): Parsed '{dt_s_processed}' to {parsed_date_only}"
            )
    ...
    if not is_date_only and date_ref and isinstance(date_ref, date):
        ...
        if re.fullmatch(
            r"^\d{1,2}:\d{1,2}(:\d{1,2})?(\s*(AM|PM))?$", dt_s_processed, re.IGNORECASE
        ):
            parsed_time_only = _parse_time_string_with_strptime(
                dt_s_processed, ctx + "_time_only_with_ref"
            )
            if parsed_time_only:
                combined_dt = datetime.combine(date_ref, parsed_time_only)
                logging.info(
                    f"{log_p} (SUCCESS - time-only with ref_date): Parsed '{dt_s_processed}' with ref_date '{date_ref}' to {combined_dt}"
                )
                return combined_dt
    ...
    m_dt_fb = re.match(
        r"(\d{1,2}:\d{1,2}(:\d{1,2})?(\s*(?:AM|PM))?)\s*(?:-\s*|,?\s+)?(.*)",
        dt_s_processed,
        re.IGNORECASE,
    )
    if m_dt_fb:
        ...
        if parsed_time_fb:
            ...
            if date_s_fallback:  # Nếu có phần date string
                parsed_date_fb = _parse_date_string_with_strptime(
                    date_s_fallback, year_use_fb, ctx + "_fb_date"
                )

            if parsed_date_fb:
                return datetime.combine(parsed_date_fb, parsed_time_fb)
            elif date_ref and isinstance(
                date_ref, date
            ):  # Nếu không parse được date_s_fallback nhưng có date_ref
                return datetime.combine(date_ref, parsed_time_fb)

    logging.warning(
        f"{log_p} (fallback or unhandled): Could not parse datetime/date from: '{dt_s}'"
    )
    return None


def parse_excel_datetime(
    dt_val,
    date_val_for_time: date | None = None,
    is_just_date: bool = False,
    context: str = "",
) -> datetime | date | None:
    ...
    if isinstance(dt_val, datetime):
        return dt_val.date() if is_just_date else dt_val
    ...
    if isinstance(dt_val, (int, float)):
        try:
            ...
            if isinstance(dt_obj_from_openpyxl, time):
                if (
                    not is_just_date
                    and date_val_for_time
                    and isinstance(date_val_for_time, date)
                ):
                ...
                return (
                    dt_obj_from_openpyxl.date()
                    if is_just_date
                    else dt_obj_from_openpyxl
                )
            else:
                logging.debug(
                    f"{log_p}: Unexpected type from openpyxl_from_excel: {type(dt_obj_from_openpyxl)}"
                )
                return None
    ...
    if isinstance(dt_val, str):
        ...
        if not dt_s or dt_s.lower() in [
            ...
            return None
        return _parse_datetime_from_string_logic(
            dt_s, date_val_for_time, is_just_date, context
        )
...
def parse_time_duration(time_val) -> float:  # Parses duration into hours (float)
    ...
    if isinstance(time_val, timedelta):
        return timedelta_to_hours(time_val)
    ...
    if isinstance(time_val, str):
        try:
            ...
            logging.warning(
                f"parse_time_duration: Cannot parse duration string: '{time_val}'"
            )
            return 0.0
    ...
    logging.warning(
        f"parse_time_duration: Unsupported type '{type(time_val)}' for duration value '{time_val}'"
    )
    return 0.0
