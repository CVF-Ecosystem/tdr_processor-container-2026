...
import pandas as pd
from datetime import datetime  # Cho isinstance check
import shutil
...
def append_df_to_excel(
    filename_path_obj: Path,
    df_to_append: pd.DataFrame,
    sheet_name_str: str,
    index=False,
    datetime_cols_to_format=None,
    time_cols_to_format=None,
    **kwargs,
) -> pd.DataFrame | None:
    """
    ...
    """
    logging.info(
        f"Bắt đầu quy trình ghi an toàn cho file: {filename_path_obj.name}, Sheet: {sheet_name_str}"
    )
        ...
        if filename_path_obj.exists():
            try:
                return pd.read_excel(
                    filename_path_obj, sheet_name=sheet_name_str, engine="openpyxl"
                )
            except Exception:
                return pd.DataFrame()
        return pd.DataFrame()
    ...
    if filename_path_obj.exists():
        try:
            ...
            logging.info(
                f"Đã đọc {len(df_existing)} dòng cũ từ sheet '{sheet_name_str}'."
            )
        except ValueError:
            logging.info(
                f"Sheet '{sheet_name_str}' không tồn tại trong file. Sẽ tạo sheet mới."
            )
        except Exception as e:
            logging.warning(
                f"Không thể đọc file '{filename_path_obj.name}'. Sẽ tạo file mới. Lỗi: {e}"
            )
    ...
    if not df_existing.empty and "Filename" in df_existing.columns:
        ...
        if before_count > after_count:
            logging.info(
                f"Đã xóa {before_count - after_count} dòng dữ liệu cũ của các file đang được cập nhật."
            )
    else:
    ...
    if not df_cleaned.columns.is_unique:
        logging.warning(
            f"Phát hiện cột trùng lặp trong dữ liệu cũ của file '{filename_path_obj.name}'. Đang xử lý..."
        )
        df_cleaned = df_cleaned.loc[:, get_unique_columns(df_cleaned)]

    if not df_to_append.columns.is_unique:
        logging.warning(
            f"Phát hiện cột trùng lặp trong dữ liệu mới cho file '{filename_path_obj.name}'. Đang xử lý..."
        )
    ...
    try:
        temp_file_path = filename_path_obj.with_suffix(
            f".{datetime.now().timestamp()}.tmp"
        )
        ...
        logging.info(
            f"Ghi đè thành công file '{filename_path_obj.name}' với {len(df_final)} dòng."
        )
        return df_final

    except Exception as e:
        logging.error(
            f"Lỗi nghiêm trọng khi ghi file Excel '{filename_path_obj.name}': {e}",
            exc_info=True,
        )
